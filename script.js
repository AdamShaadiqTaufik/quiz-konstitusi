...
const state = {
  activeUser: null,
  currentQuiz: null,
  config: { count: 50, timer: 60, mode: 'ujian' },
  questions: [],
  answers: [],
  currentIdx: 0,
  startTime: 0,
...
function renderQuestion() {
  ...
  q.options.forEach((opt, i) => {
    ...
    // In LATIHAN mode: once answered, lock and show correct/wrong markers
    // In UJIAN mode: always allow changing answer (just highlight selected)
    if (userAnswer !== null) {
      if (state.config.mode === 'latihan') {
        ...
        // Ujian mode: highlight selected but keep clickable
...
// ============================================
// HISTORY MANAGEMENT (per participant)
// ============================================
...
function saveToHistory(record) {
  try {
    ...
    // Add participant info to record
